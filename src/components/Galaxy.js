  ...
  useEffect(() => {
    ...
    function update(t) {
      ...
      if (!disableAnimation) {
        ...
        program.uniforms.uStarSpeed.value = (seconds * starSpeed) / 10.0;
      }

      const k = 0.05;
      smoothMousePos.current.x += (targetMousePos.current.x - smoothMousePos.current.x) * k;
      smoothMousePos.current.y += (targetMousePos.current.y - smoothMousePos.current.y) * k;
      smoothMouseActive.current += (targetMouseActive.current - smoothMouseActive.current) * k;
